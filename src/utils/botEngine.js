import { cardRankValue, isSuited } from './deck';
 ...
 */
export function createBotSettings(personality) {
 ...
 */
export function createOpponentSettings(personality) {
    const settings = { ...DEFAULT_SETTINGS };
    settings.personality = personality;
    switch (personality) {
        case 'tight-aggressive':
...
function preflopTier(holeCards) {
    ...
    // Premium unpaired
    if (high === 14 && low >= 11)
        tier = suited ? 9 : 8; // AK, AQ, AJ
    if (high === 14 && low === 10)
        tier = suited ? 7 : 6; // AT
    if (high === 13 && low >= 11)
        tier = suited ? 8 : 6; // KQ, KJ
    if (high === 13 && low === 10)
        tier = suited ? 6 : 5; // KT
    if (high === 12 && low === 11)
        tier = suited ? 6 : 5; // QJ
    ...
    if (high >= 10 && tier === 0)
        tier = suited ? 3 : 2;
    // Suited connectors
    if (suited && connected && high >= 7 && tier < 5)
        tier = Math.max(tier, 4);
    if (suited && connected && high >= 5 && tier < 3)
...
function positionMultiplier(position, numPlayers) {
...
export function botDecision(state, player, settings = DEFAULT_SETTINGS, opponentStats) {
    ...
    const getRaiseAmount = (strengthBonus = 0) => {
