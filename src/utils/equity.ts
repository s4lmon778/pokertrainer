 ...
 */
export function computePreflopEquity(holeCards: Card[], opponentCount: number): number {
  if (holeCards.length < 2) return 0.25;
...
export function computeEquity(
  ...
  if (communityCards.length === 0) {
    ...
    const strengthPct = Math.min(100, Math.max(0, (handEval.score / 9_500_000) * 100));
...
export function computeActionWinRates(
  player: Player,
  communityCards: Card[],
  currentPhase: GamePhase,
  currentBet: number,
  pot: number,
  activeOpponents: number = 0,
): ActionWinRates {
...
export function winRateColor(pct: number): 'green' | 'yellow' | 'red' {
...
export function winRateBgClass(pct: number): string {
  const c = winRateColor(pct);
  return c === 'green'
