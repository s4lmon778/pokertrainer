...
function highCardScore(values: number[]): number {
...
function evaluate5Card(cards: Card[]): HandEvaluation {
  ...
  const ranksOf = (count: number): number[] =>
  ...
  if (isFlush && isStraight && straightHigh === 14) {
    return { strength: 'royal-flush', score: 9_000_000, cards, description: 'Royal Flush' };
  }

  // --- Straight Flush ---
  if (isFlush && isStraight) {
    return { strength: 'straight-flush', score: 8_000_000 + straightHigh * 100, cards, description: `Straight Flush, ${RANK_NAMES[straightHigh]} high` };
  }
  ...
  if (counts[0] === 4) {
    ...
    return { strength: 'four-of-a-kind', score: 7_000_000 + (quad - 2) * 1000 + kicker * 20, cards, description: `Quad ${RANK_NAMES[quad]}` };
  }
  ...
  if (counts[0] === 3 && counts[1] >= 2) {
    ...
    return { strength: 'full-house', score: 6_000_000 + trip * 1000 + pair * 20, cards, description: `Full House, ${RANK_NAMES[trip]} full of ${RANK_NAMES[pair]}` };
  }

  // --- Flush ---
  if (isFlush) {
    return { strength: 'flush', score: 5_000_000 + highCardScore(values), cards, description: `Flush, ${RANK_NAMES[values[0]]} high` };
  }

  // --- Straight ---
  if (isStraight) {
    return { strength: 'straight', score: 4_000_000 + straightHigh * 100, cards, description: `Straight, ${RANK_NAMES[straightHigh]} high` };
  }
  ...
  if (counts[0] === 3) {
    ...
    return { strength: 'three-of-a-kind', score: 3_000_000 + (trip - 2) * 15000 + k1 * 1000 + k2 * 70, cards, description: `Trip ${RANK_NAMES[trip]}` };
  }
  ...
  if (counts[0] === 2 && counts[1] === 2) {
    ...
    const kicker = ranksOf(1)[0] || 0;
    return { strength: 'two-pair', score: 2_000_000 + (pairs[0] - 2) * 1000 + (pairs[1] - 2) * 50 + kicker, cards, description: `Two Pair, ${RANK_NAMES[pairs[0]]} and ${RANK_NAMES[pairs[1]]}` };
  }
  ...
  if (counts[0] === 2) {
    ...
    return { strength: 'pair', score: 1_000_000 + (pair - 2) * 15000 + k1 * 1000 + k2 * 70 + k3 * 5, cards, description: `Pair of ${RANK_NAMES[pair]}` };
  }
