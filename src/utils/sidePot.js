 ...
 */
export function calculateSidePots(allPlayers, communityCards) {
    ...
    for (const [pid, amt] of Object.entries(distributions)) {
        if (amt > 0)
            allWinners.push({ playerId: pid, amount: amt });
    }
    // Build side pot display
    const sidePots = pots.map(pot => ({
        amount: pot.amount,
...
//# sourceMappingURL=sidePot.js.map