 ...
 */
export function buildGameTree(settings) {
 ...
 */
function buildRootNode(settings) {
...
function createInitialState(settings) {
    return {
        ...
        stacks: [settings.startingStack, settings.startingStack],
        currentPlayer: (settings.inPositionPlayer === 0 ? 1 : 0), // Non-position acts first
        lastToAct: 0,
        currentBet: 0,
 ...
 */
function buildActionNode(parent, state, settings) {
    const node = {
        type: 'ACTION',
 ...
 */
function buildChildNode(parent, state, action, settings) {
    ...
    const bettingRoundComplete = state.currentBet === 0 &&
 ...
 */
function buildChanceNode(parent, state, street, settings) {
    const dealtCards = [];
    const node = {
        type: 'CHANCE',
        children: [],
 ...
 */
function buildTerminalNode(parent, state) {
 ...
 */
export function getLegalActions(state, settings) {
    const actions = [];
    ...
    if (remainingChips > 0 && (!facingBet || canRaiseMore)) {
        ...
        const isAllInThreshold = remainingChips <= state.pot * settings.allInThreshold;
 ...
 */
export function applyAction(state, action, settings) {
    const currentPlayer = state.currentPlayer;
    const remainingChips = state.stacks[currentPlayer];
    // Deep copy stacks
    const newState = {
    ...
    switch (action.kind) {
        ...
        case 'BET': {
            ...
            const betAmount = Math.min(Math.floor(state.pot * betSize), remainingChips);
        ...
        case 'RAISE': {
            ...
            if (state.currentBet === 0) {
                ...
                raiseAmount = Math.max(Math.floor(state.pot * raiseSize), state.minimumBet);
            }
            else {
                raiseAmount = Math.min(Math.floor(state.currentBet * (1 + raiseSize)), remainingChips + state.currentBet);
            }
    ...
    if (action.kind !== 'FOLD') {
        newState.currentPlayer = (currentPlayer === 0 ? 1 : 0);
    }
    return newState;
}
/**
...
function isTerminalState(state) {
    ...
    if (state.street === 'river') {
        ...
        const bettingComplete = state.currentBet === 0 &&
            state.lastAction !== undefined &&
            (state.lastAction === 'CHECK' || state.lastAction === 'CALL');
        if (bettingComplete) {
            return true;
 ...
 */
function computeTerminalPayoff(state) {
    const pot = state.pot;
 ...
 */
function getAvailableCards(board) {
    const available = [];
    for (let i = 0; i < 52; i++) {
        if (!board.includes(i)) {
            available.push(i);
 ...
 */
export function getTreeStats(node, settings) {
    const stats = {
        totalActionNodes: 0,
        flopActionNodes: 0,
        turnActionNodes: 0,
 ...
 */
function countNodes(node, stats, rootStreet) {
    if (node.type === 'ACTION') {
        stats.totalActionNodes++;
        // Count by street (simplified - real implementation tracks street per node)
        if (rootStreet === 'flop')
            stats.flopActionNodes++;
        else if (rootStreet === 'turn')
            stats.turnActionNodes++;
        else if (rootStreet === 'river')
            stats.riverActionNodes++;
